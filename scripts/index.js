...
const UI= (function(){
  ...
  // _ is used for the functions which we dont have to expose, whixh wil be used inside function only
	const _showMenu = () => menu.style.right = 0;

	const _hideMenu = () => menu.style.right = '-65%';

	const _toggleHourlyWeather = () => {
		let hourlyWeather = document.querySelector
		('#hourly-weather-wrapper'),
		  arrow=document.querySelector('#toggle-hourly-weather')
		  .children[0],
	...
	const drawWeatherData = (data,location) => {
		...
		dailyWeatherWrapper = document.querySelector
		('#daily-weather-wrapper'),
		dailyWeatherModel,
		day,
		maxMinTemp,
		dailyIcon,
		hourlyWeatherWrapper = document.querySelector
		('#hourly-weather-wrapper'),
		hourlyWeatherModel,
		hourlyIcon;
		...
		UI.showApp(); // after all work is done, hide the loading screen and show the application
	}
...
const GETLOCATION =(function(){
  ...
  const locationInput =document.querySelector
  						('#location-input'),
  		addCityBtn = document.querySelector
  						('#add-city-btn');
...
const WEATHER= (function(){
	...
	const darkSkyKey = '4c8ccdd0396658901b257b083c757d67',
		  geocoderKey = '076cb936926d473b9762103599c93b4b';
